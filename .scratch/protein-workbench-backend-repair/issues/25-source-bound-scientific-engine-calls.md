...
- Focused scientific-call and manifest regression suite: 98 passed / 0 failed.
- `routine`: 669 passed / 41 deselected / 0 failed.
...
- Failed SVD alignment, TM-score, and high-ambiguity tmtools tie-break invocations retain exactly one failed terminal per invoked boundary before/with the Node failure; operation details are validated before invocation, post-return decoding is covered by the same terminal boundary, and only the bounded exception type is retained.
