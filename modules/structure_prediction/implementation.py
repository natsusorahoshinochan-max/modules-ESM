...
class MaterializeConfidenceImplementation:
    ...
    def _full_join(
        ...
        fact_record = call.inputs["confidence_facts"]
        candidates = candidate_record.value
        facts = fact_record.value
        ...
        if any(
            ...
            raise ValueError(
                "materialize confidence requires structure Candidate references"
            )
        ...
        return fact_record.observation_methods[0], tuple(joined)

    def execute(self, call: OperationCall) -> dict[str, Any]:
        observation_method, joined = self._full_join(call)
