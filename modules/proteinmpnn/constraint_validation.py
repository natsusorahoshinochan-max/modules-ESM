"""Shared validation for the public ProteinMPNN constraints contract."""
...
def _validate_positions(value: Any, name: str) -> list[int]:
    if value is None:
        return []
...
def _validate_strings(value: Any, name: str) -> list[str]:
    if value is None:
        return []
    if not isinstance(value, list):
        raise ValueError(f"{name} must be a list")
...
def _validate_tied_positions(value: Any) -> list[list[int]]:
    if value is None:
        return []
    if not isinstance(value, list):
    ...
    for group_index, group_value in enumerate(value):
        group = _validate_positions(
            group_value, f"tied_positions group {group_index}"
        )
        if len(group) < 2:
            raise ValueError(
...
def _validate_biases(value: Any) -> dict[int, dict[str, float]]:
    if value is None:
        return {}
...
def _validate_combinations(
    ...
    omitted: list[str],
    tied_groups: list[list[int]],
    biases: dict[int, dict[str, float]],
) -> None:
    ...
    overlapping_chains = sorted(set(designed_chains) & set(fixed_chains))
...
def validate_constraints(constraints: ProteinMPNNConstraints) -> None:
    """Validate all structure-independent constraint rules."""
    if not isinstance(constraints, ProteinMPNNConstraints):
        raise ValueError("constraints must be ProteinMPNNConstraints")

    designable = _validate_positions(
        constraints.designable_positions, "designable_positions"
    )
    fixed = _validate_positions(constraints.fixed_positions, "fixed_positions")
    designed_chains = _validate_strings(
        constraints.designed_chains, "designed_chains"
    )
    fixed_chains = _validate_strings(constraints.fixed_chains, "fixed_chains")
    omitted = _validate_strings(
        constraints.omit_amino_acids, "omit_amino_acids"
    )
    unsupported_omissions = sorted(set(omitted) - ALPHABET)
    if unsupported_omissions:
        raise ValueError(
            "omit_amino_acids contains unsupported amino acids: "
            + ", ".join(unsupported_omissions)
        )
    if len(omitted) == len(ALPHABET):
        raise ValueError(
    ...
    _validate_combinations(
        ...
        fixed_chains,
        omitted,
        _validate_tied_positions(constraints.tied_positions),
        _validate_biases(constraints.bias_by_res),
    )
