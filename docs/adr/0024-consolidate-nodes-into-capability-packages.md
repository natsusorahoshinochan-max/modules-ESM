...
The v2 migration consolidates the current one-node directories into ten
repository-owned Module Packages: `prompt_authoring`, `esm3`, `folding`,
`proteinmpnn`, `structure_annotation`, `structure_comparison`,
`structure_transform`, `protein_io`, `selection`, and `collection_ops`.
...
form a future `solubility` Module Package.
