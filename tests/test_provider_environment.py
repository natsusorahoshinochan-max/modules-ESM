...
def test_process_environment_configures_both_solubility_providers(
    ...
    assert configuration["solubility.soluprot_no_tm.local"] == common_soluprot
    assert configuration["solubility.soluprot_full.local"] == {
        **common_soluprot,
        "perl_executable": Path(
            str(shutil.which("perl", path=os.environ["PATH"]))
        ),
    }
    assert configuration["solubility.protein_sol.local"]["source_root"] == protein_sol_root
    assert set(
        configuration["solubility.protein_sol.local"]
    ) == {"source_root", "bash_executable", "perl_executable"}
    assert all(
        "wheel_path" not in entry
        for entry in configuration.values()
    )


def test_process_environment_uses_selected_protein_sol_runtimes(
    tmp_path: Path,
) -> None:
    ...
    environment = provider_environment_configuration({
        "PATH": str(executable_root),
        "PROTEIN_WORKBENCH_PROTEIN_SOL_ROOT": str(tmp_path / "Protein-Sol"),
    })
    protein_sol = environment["solubility.protein_sol.local"]

    assert protein_sol["bash_executable"] == bash
    assert protein_sol["perl_executable"] == perl
...
def test_process_environment_configures_every_selected_real_provider(
    ...
    assert configuration["structure_annotation.dssp_compute.mkdssp_local"] == {"dssp_binary": tmp_path / "bin/mkdssp"}
