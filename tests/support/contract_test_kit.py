...
def _verify_port_cases(
    ...
    owned = {definition.type_id for definition in registration.port_types}
    verified: list[str] = []
    for case in cases:
        if case.type_id not in owned:
            raise ModulePackageConformanceError(
        ...
        try:
            ...
            if decoded != case.valid_value or port_type.encode(decoded) != encoded:
                raise ModulePackageConformanceError(
                    f"{case.type_id} codec is not canonical "
                    "and round-trippable"
                )
    ...
    *,
    ...
    try:
        ...
        result_identities = (
            ...
            if publication is not None and outputs
            else ()
        )
        if (
            ...
            or any(
                output.producer_run_id != receipt["run_id"]
...
def verify_module_package_contract(
    ...
    *,
    ...
    """Validate and execute one production registration in isolation."""
