...
def test_restart_rebuilds_selection_and_closes_all_success_run(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    catalog, project_id, workflow, roots = _selection_public_scope(
    ...
    with TestClient(
        ...
        committed = _commit_public_workflow(first, project_id, workflow)
        failed = first.post(
            f"/api/v2/projects/{project_id}/runs",
            json={
                "workflow_commit_id": committed["workflow_commit_id"],
                "client_request_id": "restart-selection-closure",
            },
        )
        later_workflow = replace(
            _workflow(catalog, "pareto"),
            workflow_id=project_id,
        )
        later_commit = _commit_public_workflow(
            first,
            project_id,
            later_workflow,
    ...
    assert later_commit["workflow_commit_id"] != committed[
        "workflow_commit_id"
    ]
    run_id = next((roots["RUN"] / project_id).iterdir()).name
    ...
    assert projection["status"] == "succeeded"
    assert len(projection["selection_results"]) == 1
    assert projection["selection_results"][0]["selection_node_id"] == (
        "select"
    )
    assert [event["event"]["type"] for event in events[-2:]] == [
        "selection_terminal",
...
def test_restart_does_not_rewrite_success_when_selection_value_is_unreadable(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    catalog, project_id, workflow, roots = _selection_public_scope(
        tmp_path,
        monkeypatch,
        "unreadable restart selection value",
    )
    ...
    with TestClient(
        ...
        failed = first.post(
            ...
            json={
                "workflow_commit_id": committed["workflow_commit_id"],
                "client_request_id": "unreadable-selection-value",
            },
        )
    ...
    selection_publication = next(
        fact["payload"]
        for fact in facts
        if fact["fact_type"] == "outputs_published"
        and fact["payload"]["node_id"] == "select"
    )

    def object_path(content_digest: str) -> Path:
        return (
            roots["OUTPUT"]
            / project_id
            / "objects"
    ...
    node_result = json.loads(
        object_path(
            selection_publication["node_result_manifest"][
                "content_digest"
            ]
        ).read_bytes()
    )
    candidates_output = next(
        output
    ...
    value_manifest = json.loads(
        object_path(
            candidates_output["value_manifest"]["content_digest"]
        ).read_bytes()
    )
    missing_value = object_path(
        value_manifest["values"][0]["content_digest"]
    )
    missing_value.unlink()
...
def test_restart_closes_selection_derivation_failure_atomically(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    catalog, project_id, workflow, roots = _selection_public_scope(
        tmp_path,
        monkeypatch,
        "restart selection derivation failure",
    )
    ...
    with TestClient(
        ...
        failed = first.post(
            ...
            json={
                "workflow_commit_id": committed["workflow_commit_id"],
                "client_request_id": "restart-selection-failure",
            },
        )
    ...
    def fail_selection_derivation(*_args: Any, **_kwargs: Any) -> None:
        raise SelectionError("restart cannot derive committed Selection")
    ...
    assert projection["selection_results"] == []
    assert projection["selection_error"]["details"] == {
        "reason": "restart cannot derive committed Selection"
    }
    closure = json.loads(
        sorted(
            (roots["RUN"] / project_id / run_id / "ledger").glob("*.json")
