...
def _install_test_provider(
    ...
    *,
    ...
    monkeypatch.setattr(
        "modules.proteinmpnn.adapter.check_proteinmpnn_readiness",
        lambda provider_root: ProteinMPNNReadiness(
            ready=True,
...
def _controlled_proteinmpnn_provider_root() -> Path:
...
def test_controlled_proteinmpnn_environment_is_test_owned() -> None:
...
def test_real_proteinmpnn_root_comes_from_environment_configuration(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    root = tmp_path / "external ProteinMPNN"
    monkeypatch.setenv("PROTEIN_WORKBENCH_PROTEINMPNN_ROOT", str(root))

    assert _proteinmpnn_provider_root() == root.resolve()
...
def test_scoring_reexecution_preserves_candidate_and_observation_identity(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    from modules.proteinmpnn.package import (
    ...
    assert len(replay_provider.parsed) == len(replay_provider.requests) == 1
    assert any(
    ...
    assert any(
        item["event"]["type"] == "engine_invocation_started"
        and item["event"]["engine_identity"] == score_method_digest
        for item in replay_events
    )
...
def test_provider_scoring_uses_exact_backbone_and_designability_mask(
    ...
    score = provider_runtime._compute_score(
        ...
        "ACD",
        torch.device("cpu"),
        _controlled_proteinmpnn_provider_root(),
    )
...
def test_proteinmpnn_readiness_rejects_linux_without_cuda(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    import sys
    ...
    monkeypatch.setattr(
        ...
        "version",
        lambda _package: adapter.PROTEINMPNN_TORCH_VERSION,
    )
    monkeypatch.setattr(
        adapter,
        "check_proteinmpnn_readiness",
        lambda *_args: pytest.fail(
            "CUDA readiness must fail before Provider checkout admission"
        ),
    )
