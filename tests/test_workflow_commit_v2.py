...
def test_commit_publish_failure_keeps_old_active_and_saved_submission(
    ...
    real_write = workflow_authoring_v2.write_private_new_file

    def fail_commit_record(root, relative_parts, payload, *, field):
...
def test_restart_rejects_tampered_commit_without_rewriting_it(
    tmp_path,
) -> None:
    project_root = tmp_path / "projects"
    projects = ProjectManager(project_root)
    ...
    catalog = _catalog()
    committed = WorkflowAuthoringService(projects, catalog).commit(
        project.id,
        expected_draft_revision=0,
        workflow=_workflow(project.id),
    )
    ...
    payload["receipt"]["catalog_contract_digest"] = "sha256:" + ("f" * 64)
    commit_path.write_text(
        json.dumps(payload, separators=(",", ":"), sort_keys=True),
        encoding="utf-8",
    )
    tampered_bytes = commit_path.read_bytes()
    ...
    assert captured.value.code == "workflow_commit_identity_mismatch"
    assert commit_path.read_bytes() == tampered_bytes


def test_restart_rejects_runnable_commit_from_another_source_draft(
    tmp_path,
) -> None:
    project_root = tmp_path / "projects"
    projects = ProjectManager(project_root)
    project = projects.create("wrong source lineage")
    catalog = _catalog()
    authoring = WorkflowAuthoringService(projects, catalog)
    committed = authoring.commit(
        project.id,
    ...
    alternative_locked = relock_workflow(
        _workflow(project.id, node_id="different-source"),
        catalog,
    )
    alternative_compiled = compile_workflow(
        alternative_locked,
        workflow_commit_revision=committed.workflow_commit_revision,
        catalog=catalog,
    )
    alternative_commit = authoring._commit_value(
        project_id=project.id,
        locked_workflow=alternative_locked,
        compiled=alternative_compiled,
        workflow_commit_revision=committed.workflow_commit_revision,
        source_draft_revision=committed.source_draft_revision,
        source_draft_digest=committed.source_draft_digest,
    )
    commit_path = next(
        (project_root / project.id / "workflow-v2" / "commits").glob(
            "*.json"
    ...
    payload = json.loads(commit_path.read_text(encoding="utf-8"))
    payload["workflow"] = alternative_locked.to_public()
    payload["receipt"] = alternative_commit.to_public()
    commit_path.write_text(
        json.dumps(payload, separators=(",", ":"), sort_keys=True),
        encoding="utf-8",
    )
    ...
    for action in ("active", "hydrate"):
        ...
        with pytest.raises(WorkflowAuthoringError) as captured:
            if action == "active":
                ...
                restarted.require_compiled(
                    project.id,
                    workflow_commit_id=alternative_commit.workflow_commit_id,
                )
...
@pytest.mark.parametrize(
    ...
    (
        ...
        ("source_draft_digest", "not-a-digest"),
        ("workflow_commit_id", "workflow-commit-not-a-digest"),
    ),
)
def test_restart_maps_malformed_commit_receipt_to_authoring_error(
    tmp_path,
    field_name: str,
    ...
    project_root = tmp_path / "projects"
    projects = ProjectManager(project_root)
    project = projects.create("malformed commit receipt")
    catalog = _catalog()
    committed = WorkflowAuthoringService(projects, catalog).commit(
        project.id,
        expected_draft_revision=0,
    ...
    payload = json.loads(commit_path.read_text(encoding="utf-8"))
    payload["receipt"][field_name] = invalid_value
    commit_path.write_text(
        json.dumps(payload, separators=(",", ":"), sort_keys=True),
        encoding="utf-8",
    )
    malformed_bytes = commit_path.read_bytes()
    ...
    for action in ("active", "hydrate"):
        ...
        assert commit_path.read_bytes() == malformed_bytes


def test_restart_rejects_commit_without_its_exact_source_draft(
    tmp_path,
) -> None:
    project_root = tmp_path / "projects"
    projects = ProjectManager(project_root)
    project = projects.create("missing source draft")
    catalog = _catalog()
    committed = WorkflowAuthoringService(projects, catalog).commit(
        project.id,
        expected_draft_revision=0,
    ...
    draft_path = next(
        (project_root / project.id / "workflow-v2" / "drafts").glob(
            "*.json"
        )
    )
    draft_path.unlink()
...
@pytest.mark.parametrize(
    "corruption",
    ("boolean_revision", "foreign_project"),
)
def test_damaged_source_draft_fails_closed_across_all_owner_reads(
    tmp_path,
    corruption: str,
) -> None:
    project_root = tmp_path / "projects"
    projects = ProjectManager(project_root)
    project = projects.create("damaged source draft")
    catalog = _catalog()
    workflow = _workflow(project.id)
    committed = WorkflowAuthoringService(projects, catalog).commit(
        project.id,
    ...
    if corruption == "boolean_revision":
        payload["draft_revision"] = True
    else:
        payload["workflow"]["workflow_id"] = "foreign-project"
        payload["draft_digest"] = parse_workflow_document(
            payload["workflow"]
        ).digest
    ...
    damaged_bytes = draft_path.read_bytes()
    ...
    for action, expected_code in actions:
        ...
        with pytest.raises(WorkflowAuthoringError) as captured:
            if action == "load_draft":
                restarted.load_draft(project.id)
            elif action == "commit_retry":
                restarted.commit(
                    project.id,
                    expected_draft_revision=0,
                    workflow=workflow,
                )
            elif action == "load_active_commit":
                restarted.load_active_commit(project.id)
            else:
                restarted.require_compiled(
                    project.id,
        ...
        assert draft_path.read_bytes() == damaged_bytes


def test_public_commit_maps_unexpected_durable_failure_to_internal_error(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path,
) -> None:
    for name in ("PROJECT", "CACHE", "OUTPUT", "RUN"):
    ...
    with TestClient(app) as client:
        project_id = client.post(
            "/api/v2/projects",
            json={"name": "durable failure"},
        ).json()["id"]

        def fail_durable_write(*_args, **_kwargs) -> None:
            raise OSError("private durable failure detail")
        ...
        response = client.post(
            ...
            json={
                "expected_draft_revision": 0,
                "workflow": _workflow(project_id).to_public(),
            },
        )

    assert response.status_code == 500
    payload = response.json()
    validate_error(payload, status=500)
    assert payload["error"]["code"] == "internal_error"
    assert payload["error"]["message"] == "Internal server error"
    assert payload["error"]["details"]["incident_id"].startswith(
        "incident-"
    )
    assert "private durable failure detail" not in response.text
